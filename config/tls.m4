...
AC_DEFUN([GCC_CHECK_TLS], [
  ...
  AC_CACHE_CHECK([whether the target supports thread-local storage],
		 gcc_cv_have_tls, [
    AC_RUN_IFELSE([AC_LANG_PROGRAM([[__thread int a; int b;]], [[ return a = b; ]])],
	...
	if test "X$thread_CFLAGS" != Xfailed; then
      ...
      [dnl This is the cross-compiling case. Assume libc supports TLS if the
       ...
       AC_LINK_IFELSE([AC_LANG_PROGRAM([[__thread int a; int b; ]], [[ return a = b; ]])],
	 [chktls_save_LDFLAGS="$LDFLAGS"
...
AC_DEFUN([GCC_CHECK_CC_TLS], [
  ...
  AC_CACHE_CHECK([whether the target assembler supports thread-local storage],
		 gcc_cv_have_cc_tls, [
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[__thread int a; int b; ]], [[ return a = b; ]])],
