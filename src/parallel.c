...
#if USE_LITHE
{
  ...
  if (specified == 1) {
    ...
    specified = gomp_global_icv.nthreads_var;
  } else if (specified > gomp_global_icv.nthreads_var) {
    gomp_fatal("gomp_resolve_num_threads has an incomplete implementation (requesting more threads than OMP_NUM_THREADS)");
  }
